...
class RP46TipiSpesaSostenutaPerRecuperoPatrimonioEdilizioInterventiAntisismici(Enum):
...
class RP46TipiInterventiParticolariPerRecuperoPatrimonioEdilizioInterventiAntisismici(Enum):
...
class RP46TipiCodiceEreditarietaInterventiDonazioniPerRecuperoPatrimonioEdilizioInterventiAntisismici(Enum):
...
class RP46NumeriRateInterventiDonazioniPerRecuperoPatrimonioEdilizioInterventiAntisismici(Enum):
...
class RP46_anno_in_cui_sono_state_sostenute_spese_per_recupero_patrimonio_edilizio_interventi_antisismici(Variable):
    value_type = int
    entity = Persona
    definition_period = YEAR # tra il 2008 e il 2017
    label = u"RP46 Col.1 - Indicare l’anno in cui sono state sostenute le spese per interventi di recupero del patrimonio edilizio e misure antisismiche"


class RP46_codice_2012_2013_2017_antisismico(Variable):
    value_type = Enum
    possible_values = RP46TipiSpesaSostenutaPerRecuperoPatrimonioEdilizioInterventiAntisismici
    default_value = RP46TipiSpesaSostenutaPerRecuperoPatrimonioEdilizioInterventiAntisismici.nessun_codice
    entity = Persona
    definition_period = YEAR
    label = u"RP46 Col.2 - Indicare codice della spesa"


class RP46_codice_fiscale(Variable):
    value_type = str
    entity = Persona
    definition_period = YEAR
...
class RP46_interventi_particolari(Variable):
    value_type = Enum
    possible_values = RP46TipiInterventiParticolariPerRecuperoPatrimonioEdilizioInterventiAntisismici
    default_value = RP46TipiInterventiParticolariPerRecuperoPatrimonioEdilizioInterventiAntisismici.nessun_codice
    entity = Persona
    definition_period = YEAR
    label = u"RP46 Col.4 - Indicare codice intervento particolare"


class RP46_acquisto_ereditarieta_donazione(Variable):
    value_type = Enum
    possible_values = RP46TipiCodiceEreditarietaInterventiDonazioniPerRecuperoPatrimonioEdilizioInterventiAntisismici
    default_value = RP46TipiCodiceEreditarietaInterventiDonazioniPerRecuperoPatrimonioEdilizioInterventiAntisismici.nessun_codice
    entity = Persona
    definition_period = YEAR
    label = u"RP46 Col.5 - Indicare codice acquisto eraditarietà donazione"


class RP46_Anno_rata(Variable):
    value_type = Enum
    possible_values = RP46NumeriRateInterventiDonazioniPerRecuperoPatrimonioEdilizioInterventiAntisismici
    default_value = RP46NumeriRateInterventiDonazioniPerRecuperoPatrimonioEdilizioInterventiAntisismici.nessun_codice
    entity = Persona
    definition_period = YEAR
    label = u"RP46 Col.8 - indicare il numero della rata che il contribuente utilizza per il 2017. Per le spese sostenute nel 2017 va obbligatoriamente indicato il numero ‘1’"


class RP46_importo_spesa(Variable):
    value_type = float
    entity = Persona
    definition_period = YEAR
    label = u"RP46 Col.9 -indicare l’intero importo delle spese sostenute nell’anno riportato in colonna 1"


class RP46_importo_rata(Variable):
    value_type = float
    entity = Persona
    definition_period = YEAR
    label = u"RP46 Col.10 -indicare l’importo di ciascuna rata delle spese sostenute. Tale importo si ottiene dividendo l’ammontare di colonna 9 (spesa sostenuta nei limiti sopra indicati), per dieci"

    def formula(person,period,parameters):
         return person('RP46_importo_spesa',period)/10.0


class RP46_numero_ordine(Variable):
    value_type = str
    entity = Persona
    definition_period = YEAR
    label = u"RP46 Col.11 -Nella presente colonna 11 va indicato un numero progressivo per identificare l’immobile oggetto degli interventi di ristrutturazione. Riportare lo stesso numero anche nella colonna 1 della seguente sezione III-B."
