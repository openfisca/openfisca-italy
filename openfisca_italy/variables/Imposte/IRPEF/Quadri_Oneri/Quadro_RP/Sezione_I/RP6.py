...
class RP6_AnnoRataDellaDetrazioneSpeseRateizzateInPrecedenza(Enum):
...
class RP6_anno_rata_sulla_detrazione_per_spese_rateizzate_in_precedenza(Variable):
    value_type = Enum
    possible_values = RP6_AnnoRataDellaDetrazioneSpeseRateizzateInPrecedenza
    default_value = RP6_AnnoRataDellaDetrazioneSpeseRateizzateInPrecedenza.nessun_numero  # The default is codice_uno
    entity = Persona
    definition_period = YEAR
    label = "RP6 col.1 - Anno della rata "
...
class RP6_spese_sanitarie_raetizzate_in_precedenza(Variable):
    value_type = float
    entity = Persona
    definition_period = YEAR
    set_input = set_input_divide_by_period
    label = "Rigo RP6 col.2 - Spese sanitarie rateizzate in precedenza: Indicare l'importo della rata spettante per quest'anno (reperibile nei modelli dei redditi precedenti)"
    reference = "http://www.agenziaentrate.gov.it/wps/file/Nsilib/Nsi/Schede/Dichiarazioni/Redditi+Persone+fisiche+2018/Modello+e+istruzioni+Redditi+PF2018/Istruzioni+Redditi+Pf+-+Fascicolo+1+2018/PF1_istruzioni_2018_Ret.pdf#page=54"  # Always use the most official source
